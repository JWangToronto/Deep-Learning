...
def testing_file(test_file):
    ...
    data['Predicted_placements'] = gnb.predict(data.values)
    df = pd.read_csv(data_path + test_file, header=None)
    df.index = df[1]
    df = pd.concat([df, data[['Predicted_placements']]], join='inner', axis=1)
    df = df.reset_index(drop=True)
    df.to_csv(data_path + test_file[:-4]+'-with-predicted-placements-by-bayes.csv', sep=',')
    return
...
#way 1: Bayes classifier
from sklearn.naive_bayes import GaussianNB
gnb = GaussianNB()
y_pred = gnb.fit(X, y).predict(X)
print("accuracy=",str((1-(((y != y_pred).sum())/X.shape[0]))*100),'%')
